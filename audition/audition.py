        ...
        for voice in candidates:
            for script in scripts:
                ...
                try:
                    ...
                    utmos_score = safe_predict_mos(str(path) if path else None)
                except ImportError:
                    utmos_score = None
                clip = {
...
def run_audition(brief: str, num_candidates: int = 8, output_dir: str | None = None,
    ...
    if scorecard and scorecard[0].get("total", 0) > 0:
        top = scorecard[0]
        mos_str = f" | MOS: {top['mos_avg']:.1f}/5" if top.get("mos_avg") else ""
        print(f"[audition] RECOMMENDATION: {top.get('name')} ({top.get('provider')}) — {top['total']}/10{mos_str}")
