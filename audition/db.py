    ...
    with connect_catalog(catalog_db_path) as conn:
        conn.executemany(
            ...
            INSERT INTO providers (
                id, name, payload_json, estimated_cost_per_min_usd, latency_tier, pricing_updated, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
...
def _provider_defaults(provider: str, conn: sqlite3.Connection) -> dict[str, Any]:
    row = conn.execute(
        """
        SELECT payload_json, estimated_cost_per_min_usd, latency_tier
    ...
    with connect_catalog(catalog_db_path) as conn:
        ...
        conn.executemany(
            ...
            INSERT INTO voices (
                ...
                effective_latency_tier, payload_json, status, metadata_source,
                enrichment_status, enrichment_attempts, first_seen, last_seen, last_synced,
...
def get_acoustic_features(voice_id: str, catalog_db_path: str | Path | None = None) -> dict | None:
    ...
    with connect_catalog(catalog_db_path) as conn:
        row = conn.execute(
            """
            SELECT f0_mean_hz, f0_std_hz, speech_rate_syl_per_sec, hnr_db, spectral_centroid_hz, updated_at
