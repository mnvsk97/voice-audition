...
from audition.db import get_acoustic_features, get_voice as _get_voice
from audition.index import search_voices as _search_voices
from audition.search import compute_catalog_stats, filter_voices as _filter_voices, load_all_voices
...
def _simplify(v: dict, **extra) -> dict:
    out = {
        k: v.get(k)
        for k in ("id", "name", "provider", "gender", "description", "effective_cost_per_min_usd", "effective_latency_tier", "effective_quality_elo")
    }
    out.update(extra)
    return out
